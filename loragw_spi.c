...
int lgw_spi_open(void **spi_target_ptr) {
    ...
    if ((a < 0) || (b < 0)) {
    	DEBUG_ERROR("ERROR: SPI PORT FAIL TO SET IN MODE 0\n");
        close(dev);
        free(spi_device);
        return LGW_SPI_ERROR;
    }
    ...
    if ((a < 0) || (b < 0)) {
    	DEBUG_ERROR("ERROR: SPI PORT FAIL TO SET MAX SPEED\n");
        close(dev);
        free(spi_device);
        return LGW_SPI_ERROR;
    }
    ...
    if ((a < 0) || (b < 0)) {
    	DEBUG_ERROR("ERROR: SPI PORT FAIL TO SET MSB FIRST\n");
        close(dev);
        free(spi_device);
        return LGW_SPI_ERROR;
    }
    ...
    if ((a < 0) || (b < 0)) {
    	DEBUG_ERROR("ERROR: SPI PORT FAIL TO SET 8 BITS-PER-WORD\n");
        close(dev);
        return LGW_SPI_ERROR;
    }
...
int lgw_spi_close(void *spi_target) {
    ...
    if (a < 0) {
    	DEBUG_ERROR("ERROR: SPI PORT FAILED TO CLOSE\n");
        return LGW_SPI_ERROR;
    } else {
...
/* Simple write */ //                   mode_0             0x0                    0xaa             0x96
int lgw_spi_w(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, uint8_t address, uint8_t data) {
    ...
    CHECK_NULL(spi_target);
    if ((address & 0x80) != 0) {
    	DEBUG_ERROR("WARNING: SPI address > 127\n");
    }
    ...
    /* determine return code */
    if (a != (int)k.len) {
        DEBUG_ERROR("ERROR: SPI WRITE FAILURE\n");
        return LGW_SPI_ERROR;
    } else {
...
int lgw_spi_r(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, uint8_t address, uint8_t *data) {
    ...
    /* determine return code */
    if (a != (int)k.len) {
    	DEBUG_ERROR("ERROR: SPI READ FAILURE\n");
        return LGW_SPI_ERROR;
    } else {
...
int lgw_spi_wb(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, uint8_t address, uint8_t *data, uint16_t size) {
    ...
    CHECK_NULL(data);
    if (size == 0) {
    	DEBUG_ERROR("ERROR: BURST OF NULL LENGTH\n");
        return LGW_SPI_ERROR;
    }
    ...
    if (byte_transfered != size) {
    	DEBUG_ERROR("ERROR: SPI BURST WRITE FAILURE\n");
        return LGW_SPI_ERROR;
    } else {
...
//    	printf("szie = %d\n",size);
    ...
    /* determine return code */
    if (byte_transfered != size) {
    	DEBUG_ERROR("ERROR: SPI BURST READ FAILURE\n");
        return LGW_SPI_ERROR;
    } else {
