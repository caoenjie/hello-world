...
{
	lgw_spi_w(SX1276.Spi, spi_mux_mode, spi_mux_target, addr, data);
}

uint8_t SX1276Read( uint16_t addr )
{
    uint8_t data;
    lgw_spi_r(SX1276.Spi, spi_mux_mode, spi_mux_target, addr, &data);
    return data;
}

void SX1276WriteBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
    lgw_spi_wb(SX1276.Spi, spi_mux_mode, spi_mux_target, addr, buffer, size);
}

void SX1276ReadBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
	lgw_spi_rb(SX1276.Spi, spi_mux_mode, spi_mux_target, addr, buffer, size);
}
...
void SX1276OnTimeoutIrq( void* context )
{
	printf("SX1276OnTimeoutIrq function!\n");
...
void SX1276OnDio1Irq( void* context )
{
	printf("SX1276OnDio1Irq function!\n");
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            switch( SX1276.Settings.Modem )
...
void SX1276CheckDIO(void)
{
	if(DIO0) SX1276OnDio0Irq(NULL);
	if(DIO1) SX1276OnDio1Irq(NULL);
	if(DIO2) SX1276OnDio2Irq(NULL);
	if(DIO3) SX1276OnDio3Irq(NULL);
	if(DIO5) SX1276OnDio5Irq(NULL);
